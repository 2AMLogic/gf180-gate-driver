...
# ============================================================================
# github_app_invalidate_cache (#51): removes cached installation tokens +
# owner->installation mappings so the next get-token call re-resolves/re-mints
# from scratch -- the in-script alternative to an out-of-repo `rm -f` on
# ~/.cache/loom/github-app/ that the guard's rm-scope-outside-repo pattern
# denies.
# ============================================================================
echo "Testing github_app_invalidate_cache (#51)..."

INVALIDATE_CACHE_DIR="$WORK_DIR/invalidate-cache-test"
mkdir -p "$INVALIDATE_CACHE_DIR"
export LOOM_GITHUB_APP_CACHE_DIR="$INVALIDATE_CACHE_DIR"

# Populate a token cache and an owner->installation mapping, plus an unrelated
# file that must survive (scope check: only the two known glob patterns are
# touched -- never an arbitrary sweep of the cache dir).
_github_app_write_cache "888" "ghs_shouldberemoved" "2099-01-01T00:00:00Z"
echo "888" > "$(_github_app_installation_cache_path "some-owner")"
echo "unrelated" > "$INVALIDATE_CACHE_DIR/not-a-cache-file.txt"
token_cache_path="$(_github_app_cache_path "888")"
owner_cache_path="$(_github_app_installation_cache_path "some-owner")"

github_app_invalidate_cache

if [[ ! -f "$token_cache_path" ]]; then
    pass "invalidate_cache removes the cached installation token file"
else
    fail "invalidate_cache should remove the token cache file"
fi
if [[ ! -f "$owner_cache_path" ]]; then
    pass "invalidate_cache removes the owner->installation mapping file"
else
    fail "invalidate_cache should remove the owner mapping file"
fi
if [[ -f "$INVALIDATE_CACHE_DIR/not-a-cache-file.txt" ]]; then
    pass "invalidate_cache leaves non-cache files in the cache dir untouched (narrow glob scope)"
else
    fail "invalidate_cache should not touch files outside its fixed glob"
fi
if [[ "$GITHUB_APP_CACHE_FILES_REMOVED" == "2" ]]; then
    pass "invalidate_cache reports the correct removed-file count"
else
    fail "invalidate_cache should report 2 files removed" "got: $GITHUB_APP_CACHE_FILES_REMOVED"
fi

# Re-running against an already-empty cache dir is a successful no-op (0
# removed, not an error) -- the common case when nothing was cached.
github_app_invalidate_cache
if [[ "$GITHUB_APP_CACHE_FILES_REMOVED" == "0" ]]; then
    pass "invalidate_cache on an already-empty cache dir is a successful no-op (0 removed)"
else
    fail "invalidate_cache on an empty cache dir should report 0 removed" "got: $GITHUB_APP_CACHE_FILES_REMOVED"
fi

# CLI dispatch: `invalidate-cache` subcommand exits via the JSON envelope,
# reports the count, works even when no app is configured (a maintenance op
# on files this script owns, independent of current app config), and leaks no
# secret material.
CLI_CACHE_DIR="$WORK_DIR/invalidate-cache-cli"
mkdir -p "$CLI_CACHE_DIR"
LOOM_GITHUB_APP_CACHE_DIR="$CLI_CACHE_DIR" _github_app_write_cache "999" "ghs_clisecretvalue" "2099-01-01T00:00:00Z"
cli_token_cache_path="$(LOOM_GITHUB_APP_CACHE_DIR="$CLI_CACHE_DIR" _github_app_cache_path "999")"

cli_invalidate_output=$(LOOM_GITHUB_APP_CACHE_DIR="$CLI_CACHE_DIR" LOOM_CONFIG_DEFAULTS_FILE="" \
  bash "$LIB_DIR/github-app-token.sh" invalidate-cache)
cli_invalidate_status=$(echo "$cli_invalidate_output" | jq -r '.status')
cli_invalidate_removed=$(echo "$cli_invalidate_output" | jq -r '.removed')
if [[ "$cli_invalidate_status" == "ok" && "$cli_invalidate_removed" == "1" ]]; then
    pass "\`invalidate-cache\` CLI reports ok with the correct removed count"
else
    fail "\`invalidate-cache\` CLI should report ok/removed=1" "$cli_invalidate_output"
fi
if [[ ! -f "$cli_token_cache_path" ]]; then
    pass "\`invalidate-cache\` CLI actually deletes the on-disk cache file"
else
    fail "\`invalidate-cache\` CLI left the token cache file in place"
fi
if [[ "$cli_invalidate_output" != *"ghs_clisecretvalue"* ]]; then
    pass "\`invalidate-cache\` CLI output contains no token material"
else
    fail "\`invalidate-cache\` CLI leaked secret material" "$cli_invalidate_output"
fi

# Restore the shared test cache dir for the sections below.
export LOOM_GITHUB_APP_CACHE_DIR="$WORK_DIR/cache"
