...
Four separate Judge `changes-requested` verdicts (PRs #19, #57, #60, #92 —
all same-day fix-and-reverify, so nothing bad reached `main`, but each cost
a review round-trip) found a real bypass in a `guard-destructive-generic.sh`
...
edge cases, a heredoc delimiter parser that only handled a single
quoted segment (missing the valid-bash multi-segment form `<<'E'O'F'`,
which desynchronized masking and let a `rm -rf /` line through), and a
live `git clean -fd` allowed through because it sat inside a `$(...)`
command substitution without being the first token after the opener. In
each case Judge had to invent adversarial inputs beyond the PR's stated
...
these four boundary shapes in addition to the literal reported repro —
they account for all four real bypasses above:
...
- a case where two separate spans appear on the same command line
  (cross-span bridging),
- a case where the target appears inside a command substitution (`$(...)`
  or backticks) at a position other than the first recognized token — e.g.
  `echo "$(true; git clean -fd .)"` — since a scanner that only checks the
  first token after the opener (or that flattens the whole quoted span into
  one token) silently misses it.
