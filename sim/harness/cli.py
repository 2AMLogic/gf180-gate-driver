...
Ported from `2AMLogic/gf180-bandgap` (sim/harness/cli.py). The single-rail
``--supply``/``--supply-tol`` overrides are dropped: this repo's testbenches
declare their rails (default: ``vlogic`` 3.3 V, ``vdrv`` 5 V -- see
``corners.DEFAULT_RAILS``) in the manifest, and the two rails are swept
together (``corners.tied_supply_grid``), so there is no single scalar left to
override from the command line the way the source harness's one ``vdd`` was.
"""
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        ...
        epilog=(
            "Examples:\n"
            "  python3 sim/run_corners.py smoke-mv-inverter\n"
    ...
    parser.add_argument(
        "--claim",
        default="",
        help="spec line this run substantiates, e.g. 'spec/gate-driver.md#section' "
    ...
    parser.add_argument(
        "--version", action="version", version=f"gf180-gate-driver harness {HARNESS_VERSION}"
    )
    return parser
...
def cmd_list() -> int:
    ...
    print("\nDefault rails:")
    for rail in corners_mod.DEFAULT_RAILS:
        extra = f" (+ stretch: {', '.join(f'{v:g}V' for v in rail.extra_v)})" if rail.extra_v else ""
        print(f"  {rail.name:<10} {rail.nominal_v:g} V +/-{rail.tolerance * 100:g}%{extra}")
    return EXIT_OK
...
def run(args: argparse.Namespace) -> int:
    ...
    supply_grid = corners_mod.tied_supply_grid(tb.rails)
    if tb.stretch:
        supply_grid = supply_grid + corners_mod.stretch_points(tb.rails)
    points = corners_mod.build_grid(corner_list, temperatures, supply_grid)
    ...
    def progress(result):
        ...
        print(f"[{completed:>3}/{len(points)}] {flag} {result.point.corner_id:<40} {detail}")
