...
Ported from `2AMLogic/gf180-bandgap` (sim/harness/testbench.py) and adapted
for this repo's two-rail (``vlogic`` / ``vdrv``) supply axis -- see
``sim/harness/corners.py``.
...
can be swept across the whole PVT grid without editing. The harness hands the
fragment, per rail declared in the manifest's ``rails`` map (default: this
repo's two rails, ``vlogic`` and ``vdrv`` -- see ``corners.DEFAULT_RAILS``):

    <rail>_val   the supply for this PVT point (nominal, +tol or -tol)
    <rail>_nom   the rail's nominal supply, for ratio-style measurements

plus ``temp_c`` (also applied via ``.temp``) and anything in the manifest's
``params`` map.

A manifest may also name a **device under test**::

    sim/<experiment-slug>/testbench/tb.json   {"dut": "sim/dut/gate_driver_top.spice"}
    ...
    *presence*, not by truthiness of the value.
    ...
    for name, spec in manifest_rails.items():
        rails.append(
            Rail(
                name=name,
                nominal_v=float(spec["nominal_v"]),
                tolerance=float(spec.get("tolerance", 0.10)),
                extra_v=tuple(float(v) for v in spec.get("extra_v", ())),
            )
        )
    return tuple(rails)
...
class Testbench:
    ...
    claim: str = ""
    subset_reason: str = ""
    rails: tuple[Rail, ...] = DEFAULT_RAILS
    ...
    def provenance(self) -> dict:
        return {
            ...
            "rails": [
                {"name": r.name, "nominal_v": r.nominal_v, "tolerance": r.tolerance}
                for r in self.rails
            ],
        }
...
def load(directory: str | Path, dut: str | Path | None = None) -> Testbench:
    ...
    for rail in rails:
        if not rail.name.replace("_", "").isalnum() or not rail.name.islower():
            raise ValueError(
                f"{manifest_path}: rail name {rail.name!r} must be lowercase "
                "alphanumeric/underscore (it becomes an ngspice .param prefix "
                "and a corner-id token)"
            )
    ...
    tb = Testbench(
        ...
        stretch=bool(manifest.get("stretch", False)),
