...
corner-id grammar's supply field is adapted for this repo's two-rail sweep:
one or more ``<rail><volts>v`` tokens joined by ``-`` (see
``sim/harness/corners.py``'s ``PvtPoint.corner_id`` and ``sim/README.md``),
in place of the source harness's single, optionally-named ``<volts>v`` /
``<node><volts>v`` token.
...
#: One named-rail token: `vlogic3p30v`, `vdrv5p00v` (`p` stands in for the
#: decimal point so a multi-rail supply field, several of these joined by
#: `-`, stays a single underscore-free token -- see corners.py's
#: `PvtPoint.corner_id`). This repo always names the rail (unlike
#: gf180-bandgap's single implicit `vdd`, which allowed a bare `2.97v`).
_RAIL_TOKEN_RE = r"[a-z]+\d+(?:[.p]\d+)?v"
#: One or more rail tokens joined by `-`: `vlogic3p30v-vdrv5p00v`.
_SUPPLY_RE = re.compile(rf"^{_RAIL_TOKEN_RE}(?:-{_RAIL_TOKEN_RE})*$")
...
def parse_corner_id(corner_id: str) -> str | None:
    ...
    The grammar is the one documented in ``sim/README.md``::
        ...
        <supply>    ::= "nosupply" | <rail> ("-" <rail>)*
        <rail>      ::= name volts "v"            # vlogic3p30v, vdrv5p00v
    ...
    if supply != NO_SUPPLY and not _SUPPLY_RE.match(supply):
        return (
            f"{corner_id!r}: supply field {supply!r} is neither one or more "
            "<rail><volts>v tokens joined by '-' (vlogic3p30v, "
            f"vlogic3p30v-vdrv5p00v) nor the literal {NO_SUPPLY!r}"
        )
    return None
