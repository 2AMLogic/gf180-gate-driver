...
| Overvoltage / gate-oxide protection | **In scope, structural, with one documented exception (decision record 0003)** | Addressed by the level-shifter topology choice itself (§4) rather than as a separate protection circuit, for every thin-oxide node exposed to the 5 V/6 V drive rail — verified across the full PVT matrix (decision record 0002). **Exception**: the pre-driver inverter's own output (`inb`, gate of thin-oxide `XMNPDB`, internal to the 3.3 V logic domain and never touching the drive rail) transiently overshoots its own `VDD_LOGIC` rail by 20–35 mV, only at the `vlogic3p63v` (+10 %) PVT corner — measured worst case 3.65019 V–3.66512 V across the 15 affected process×temperature points, `sim/level-shifter-oxide-safety/records/20260808-052057-5fbdb2d.md`. This is a documented, bounded margin, not the PDK's duty-cycle TDDB overshoot allowance (§2.3, not invoked). See decision record 0003 for the mitigation attempts investigated and why the claim is scoped this way instead. |
