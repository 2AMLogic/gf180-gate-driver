...
15 stretch-rail points clear their respective targets; peak *source*
current clears 1 A everywhere on the stretch rail. **Bounded at ≥ 0.85 A**
at these two corners. This is not a relaxation of the ≥ 1 A target or of
the sim harness's per-corner check — both stay in force, and the affected
corners continue to report FAIL by design, per decision record 0016.

**Second known exception, nominal rail, `uvlo` instantiated (decision
record
[0020](decision-records/0020-uvlo-locked-corner-ipeak-source-artifact.md))**:
with `uvlo` (§5, issue #220) instantiated in the block, the nominal ±10 %
matrix's ≥ 0.5 A peak *source* current target is **not met** at exactly one
corner — `ss_-40c_vlogic2p97v-vdrv4p50v` (0.475414–0.499696 A depending on
facet). This is not an independent drive-strength weakness: at this corner
the block is already fully locked out by `uvlo`'s own (out-of-spec, per
decision record 0018 Finding 2) trip point at this legitimate −10 %
operating point, and the measured current is a sub-nanosecond
contention-current artifact between the output driver and the engaged UVLO
pulldown, not a sustained drive attempt. **Bounded at ≥ 0.45 A** at this
corner specifically. Every other nominal ±10 % point, with `uvlo`
instantiated, clears its target — the "every point of the nominal ±10 %
matrix clears their respective targets" claim above holds with this one
documented, bounded exception, not universally.
...
|---|---|---|
| UVLO (undervoltage lockout) on the 5 V/6 V drive rail | **In scope, implemented and PVT-verified, with an open false-trip finding (decision record 0001 Decisions 4–5; measured and superseded in part by decision record 0018)** | Operating the thick-oxide output stage and level-shifter cascode below a valid `Vgs` collapses drive strength and increases switching loss without tripping any other protection; cheap to add and directly protects the medium-voltage devices this block exists to characterize. `design/uvlo.sch` (issue #220) implements decision record 0001 Decision 5's topology (resistive divider off `VDD_DRV` vs. a diode-connected `nfet_06v0` `Vt` reference — no bandgap) and is instantiated in `design/gate_driver_core.sch` as `x3`. Measured full-PVT-grid trip thresholds (`sim/uvlo-trip-verification/`) are substantially wider than decision record 0001 Decision 4's stated worst-case ranges — falling 2.231–4.737 V (vs. stated 3.3–3.9 V), rising 2.503–5.115 V (vs. stated 3.6–4.2 V) — root-caused by decision record 0018 to the divider's ~5× amplification of the diode reference's own process *and temperature* spread, an effect decision record 0001 under-weighted. Response time (< 240 ns at every corner, target < 500 ns) and the guaranteed-off claim (`VDD_DRV` < 3.3 V forces lockout, verified with margin down to 2.0 V) are both **confirmed**. The guaranteed-on claim (`VDD_DRV` > 4.2 V releases) is **not confirmed**: at the `ss`/−40 °C corner the block remains locked out up to 5.115 V, above the drive rail's own −10 % low-line floor (4.50 V) — a real false-trip risk in otherwise-legitimate operation, corroborated independently in the full-block context (`sim/gate-driver-core-drive-with-uvlo/`). This is left as an **open, unresolved safety finding** (not a bounded exception) pending a future reference-topology revision — see decision record 0018 for the full analysis and why a fix is deferred rather than attempted in issue #220. This same false-trip lockout also produces a §3 drive-strength consequence at one nominal-rail corner (`ss_-40c_vlogic2p97v-vdrv4p50v`) — see §3's own second known exception and decision record 0020. |
